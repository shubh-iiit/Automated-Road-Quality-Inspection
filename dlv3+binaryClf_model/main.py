...
def get_argparser():
    ...
    # Dataset Options
    parser.add_argument("--data_root", type=str, default='/home/shubhamp/Downloads/Seg_Emarg15k_val1500',
...
def validate(opts, model, loader, device, binary_metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        ...
        with open("validation15k.txt", "w") as file:
            ...
            wandb.log({"TP": metrics["TP"], "Step": i})
            wandb.log({"TN": metrics["TN"], "Step": i})
            wandb.log({"FP": metrics["FP"], "Step": i})
            wandb.log({"FN": metrics["FN"], "Step": i})
...
def main():
    ...
    # Initialize wandb
    wandb.init(project="classificationemarg15k_resnet50backbone_6classes_Exp6", name="metric_plots", config=vars(opts))
    ...
    # print("Keys in backbone model state_dict:")
    # print(backbone_model.state_dict().keys())

    # Get all keys from the backbone model's state dictionary
    backbone_keys = [k for k in backbone_model.state_dict().keys()]

    # Filter the keys to retain only those related to the backbone
    backbone_keys = [k for k in backbone_keys if k.startswith('backbone.')]

    # Print the filtered backbone keys
    # print("Backbone keys:")
    # print(backbone_keys)
